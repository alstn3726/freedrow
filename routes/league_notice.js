...
router.get('/write',(req,res)=>{
    ...
    console.log('삽입 페이지');
    res.render('league_notice/write', {
...
router.post('/upload', upload.single('files'), (req, res) => {
    ...
    if(req.file ==  undefined) {
        client.query('insert into l_notice (board_id,subject,content,reg_date) values(?,?,?,?)', 
        ...
        client.query('insert into l_notice (board_id,subject,content,reg_date,image) values(?,?,?,?,?)', 
...
router.get('/:num',(req,res,next)=>{
    ...
    client.query('select * from l_notice order by board_num desc ', (error, data) => {
        res.render('league_notice/list', { 
            data: data,
            page:page,
            leng:data.length-1,
            page_num:10,
...
router.get('/detail/:board_num',(req,res)=>{
    ...
    const body = req.body;
    client.query('select * from l_notice where board_num=?',[req.params.board_num],(err,data)=>{
        console.log('댓글 list');
        client.query('select * from re_l_notice where parent=? order by re_num desc',[req.params.board_num],(err,rdata)=>{
        console.log(err);
        res.render('league_notice/detail',{
            data : data[0],
            rdata : rdata,
            logined : req.session.userid,
            moment : moment,
    ...
    //조회수 기능
    client.query('update l_notice set hit=hit+1 where board_num=?',[req.params.board_num]); 
});
...
router.get('/update/:board_num',(req,res)=>{
    ...
    client.query('select * from l_notice where board_num=?',[req.params.board_num],(err,data)=>{
        ...
        res.render('league_notice/update',{
            data : data[0],
            logined : req.session.userid,
        })
    })
...
router.post('/update/:board_num',(req,res)=>{
    ...
    client.query('update l_notice set subject=?, content=? where board_num=?',
    [body.subject, body.content, req.params.board_num],(err,data)=>{
        console.log(err);
        console.log('data ==>',data);
        res.send("<script>location.replace('/league_notice/list/1')</script>");
    })
})
...
router.get('/delete/:board_num',(req,res)=>{
    console.log('삭제 진행');
    client.query('delete from l_notice where board_num=?',[req.params.board_num],(err,data)=>{
        console.log(err);
        console.log('data ==>',data);
        res.send("<script>location.replace('/league_notice/list/1')</script>");
    });
...
router.post('/detail/:board_num',(req,res)=>{
    ...
    console.log(req.session);
    const body = req.body;
    client.query('insert into re_l_notice(parent,re_id,re_content,re_date)values(?,?,?,?)',
...
router.get('/redelete/:board_num',(req,res)=>{
    console.log('댓글삭제 진행');
    client.query('delete from re_l_notice where re_num=?',[req.params.board_num],(err,rdata)=>{
        console.log(err);
...
router.get('/re_update/:re_num',(req,res)=>{
    console.log('수정 페이지 진행');
    client.query('select * from re_l_notice where re_num=?',[req.params.re_num],(err,data)=>{
        console.log(err);
        res.render('league_notice/re_update',{
            data : data[0],
            logined : req.session.userid,
        })
    })
...
router.post('/re_update/:re_num',(req,res)=>{
    ...
    client.query('update re_l_notice set re_content=? where re_num=?',
