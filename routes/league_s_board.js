...
router.get('/write',(req,res)=>{
    ...
    console.log('삽입 페이지');
    res.render('league_s_board/write', {
...
router.post('/upload', upload.single('files'), (req, res) => {
    ...
    if(req.file ==  undefined) {
        client.query('insert into s_l_board (board_id,subject,content,reg_date) values(?,?,?,?)', 
        ...
        client.query('insert into s_l_board (board_id,subject,content,reg_date,image) values(?,?,?,?,?)', 
...
router.get('/:num',(req,res,next)=>{
    ...
    client.query('select * from s_l_board order by board_num desc ', (error, data) => {
        res.render('league_s_board/list', { 
            data: data,
            page:page,
            leng:data.length-1,
            page_num:10,
...
router.get('/detail/:board_num',(req,res)=>{
    ...
    const body = req.body;
    client.query('select * from s_l_board where board_num=?',[req.params.board_num],(err,data)=>{
        console.log('댓글 list');
        client.query('select * from re_s_l_board where parent=? order by re_num desc',[req.params.board_num],(err,rdata)=>{
        console.log(err);
        res.render('league_s_board/detail',{
            data : data[0],
            rdata : rdata,
            logined : req.session.userid,
            moment : moment,
    ...
    //조회수 기능
    client.query('update s_l_board set hit=hit+1 where board_num=?',[req.params.board_num]); 
});
...
router.get('/update/:board_num',(req,res)=>{
    ...
    client.query('select * from s_l_board where board_num=?',[req.params.board_num],(err,data)=>{
        ...
        res.render('league_s_board/update',{
            data : data[0],
            logined : req.session.userid,
        })
    })
...
router.post('/update/:board_num',(req,res)=>{
    ...
    client.query('update s_l_board set subject=?, content=? where board_num=?',
    [body.subject, body.content, req.params.board_num],(err,data)=>{
        console.log(err);
        console.log('data ==>',data);
        res.send("<script>location.replace('/league_s_board/list/1')</script>");
    })
})
...
router.get('/delete/:board_num',(req,res)=>{
    console.log('삭제 진행');
    client.query('delete from s_l_board where board_num=?',[req.params.board_num],(err,data)=>{
        console.log(err);
        console.log('data ==>',data);
        res.send("<script>location.replace('/league_s_board/list/1')</script>");
    });
...
router.post('/detail/:board_num',(req,res)=>{
    ...
    console.log(req.session);
    const body = req.body;
    client.query('insert into re_s_l_board(parent,re_id,re_content,re_date)values(?,?,?,?)',
...
router.get('/redelete/:board_num',(req,res)=>{
    console.log('댓글삭제 진행');
    client.query('delete from re_s_l_board where re_num=?',[req.params.board_num],(err,rdata)=>{
        console.log(err);
...
router.get('/re_update/:re_num',(req,res)=>{
    console.log('수정 페이지 진행');
    client.query('select * from re_s_l_board where re_num=?',[req.params.re_num],(err,data)=>{
        console.log(err);
        res.render('league_s_board/re_update',{
            data : data[0],
            logined : req.session.userid,
        })
    })
...
router.post('/re_update/:re_num',(req,res)=>{
    ...
    client.query('update re_s_l_board set re_content=? where re_num=?',
