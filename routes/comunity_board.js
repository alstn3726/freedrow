...
router.get('/write',(req,res)=>{
    ...
    console.log('삽입 페이지');
    res.render('comunity_board/write', {
...
router.post('/upload', upload.single('files'), (req, res) => {
    ...
    if(req.file ==  undefined) {
        client.query('insert into c_m_board (board_id,subject,content,reg_date) values(?,?,?,?)', 
        ...
        sharp(req.file.path).resize({width:380, height:253, position:"left top"})
        .toFile(path.dirname(__dirname) + '/public/images/picture/'+req.file.filename)
        client.query('insert into c_m_board (board_id,subject,content,reg_date,image) values(?,?,?,?,?)', 
...
router.get('/:num',(req,res,next)=>{
    ...
    client.query('select * from c_m_board order by board_num desc ', (error, data) => {
        res.render('comunity_board/list', { 
            data: data,
            page:page,
            leng:data.length-1,
            page_num:10,
...
router.get('/detail/:board_num',(req,res)=>{
    ...
    const body = req.body;
    client.query('select * from c_m_board where board_num=?',[req.params.board_num],(err,data)=>{
        console.log('댓글 list');
        client.query('select * from re_c_m_board where parent=? order by re_num desc',[req.params.board_num],(err,rdata)=>{
        console.log(err);
        res.render('comunity_board/detail',{
            data : data[0],
            rdata : rdata,
            logined : req.session.userid,
            moment : moment,
    ...
    //조회수 기능
    client.query('update c_m_board set hit=hit+1 where board_num=?',[req.params.board_num]); 
});
...
router.get('/update/:board_num',(req,res)=>{
    ...
    client.query('select * from c_m_board where board_num=?',[req.params.board_num],(err,data)=>{
        ...
        res.render('comunity_board/update',{
            data : data[0],
            logined : req.session.userid,
        })
    })
...
router.post('/update/:board_num',(req,res)=>{
    ...
    client.query('update c_m_board set subject=?, content=? where board_num=?',
    [body.subject, body.content, req.params.board_num],(err,data)=>{
        console.log(err);
        console.log('data ==>',data);
        res.send("<script>location.replace('/comunity_board/list/1')</script>");
    })
})
...
router.get('/delete/:board_num',(req,res)=>{
    console.log('삭제 진행');
    client.query('delete from c_m_board where board_num=?',[req.params.board_num],(err,data)=>{
        console.log(err);
        console.log('data ==>',data);
        res.send("<script>location.replace('/comunity_board/list/1')</script>");
    });
...
router.post('/detail/:board_num',(req,res)=>{
    ...
    console.log(req.session);
    const body = req.body;
    client.query('insert into re_c_m_board(parent,re_id,re_content,re_date)values(?,?,?,?)',
...
router.get('/redelete/:board_num',(req,res)=>{
    console.log('댓글삭제 진행');
    client.query('delete from re_c_m_board where re_num=?',[req.params.board_num],(err,rdata)=>{
        console.log(err);
...
router.get('/re_update/:re_num',(req,res)=>{
    console.log('수정 페이지 진행');
    client.query('select * from re_c_m_board where re_num=?',[req.params.re_num],(err,data)=>{
        console.log(err);
        res.render('comunity_board/re_update',{
            data : data[0],
            logined : req.session.userid,
        })
    })
...
router.post('/re_update/:re_num',(req,res)=>{
    ...
    client.query('update re_c_m_board set re_content=? where re_num=?',
